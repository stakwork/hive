...
Status: **complete** (Phases 1–4 shipped). Web work done; the iOS-side `SubAgentRunCard` / `PlannerFormSlot` renderers remain a parallel task for the iOS team.
...
> - ✅ **Phase 4** — `PlannerFormSlot` (`src/app/org/[githubLogin]/_components/PlannerFormSlot.tsx`) + `POST /api/orgs/[githubLogin]/planner-forms/answer` endpoint + `user-answered-planner-form` extractor arm in `SubAgentRunCard` + `pendingForm` derivation + `PlannerFormSlot` mount in `SidebarChat`. Shipped. See **Phase 4 implementation notes** below.
>
> **Critical representation correction (Phases 3+4).** A feature planner asks the user a structured question via a **`PLAN` artifact whose content passes `isClarifyingQuestions`** (`tool_use === "ask_clarifying_questions"`, `content: ClarifyingQuestion[]`) — the shape `ClarifyingQuestionsPreview` renders. This is **NOT** an `ArtifactType.FORM` (that's the *task* chat's form). Phase 3 originally shipped with `ArtifactType.FORM` detection in `actionableWakeReason` / the fan-out `hasForm`; Phase 4 corrected both to the PLAN-clarifying-questions representation via the shared `extractClarifyingQuestions(plannerMessage)` helper in `canvas-planner-fanout.ts`.
>
...
| `src/services/canvas-planner-fanout.ts` | ✅ **New.** ~265 lines. Owns the fan-out worker: short-circuits when no parent; opens `db.$transaction` with `SELECT ... FOR UPDATE` on `shared_conversations`; idempotency-checks via `source.plannerMessageId`; appends a `planner`-source `CanvasChatMessage` row to `messages` JSON; bumps `lastMessageAt`. Failure-tolerant (caught + logged at top level). On a fresh append it fires a `CANVAS_CONVERSATION_UPDATED` Pusher nudge and **returns the `actionableWakeReason`** (FORM → `form`; terminal `workflowStatus` → `completed`/`failed`/`halted`; trailing-`?` → `question`; else `null`). It no longer invokes the auto-turn itself — the webhook route schedules `invokeCanvasAgentOnPlannerMessage` in `after()` so the LLM turn runs off the webhook's critical path. | — |
...
| `src/app/org/[githubLogin]/_components/SubAgentRunCard.tsx` | ✅ Phase 1 — collapsed-by-default with one-line headline. ✅ Phase 2 — extractor reads inbound `source.kind === "planner"` messages and threads them with `direction: "in"`, anchor advances to the freshest entry, render alternates `→` / `←`, headline says `"Replied"` when latest entry is inbound, status icon switches to a green check. ✅ Phase 3 — `deriveCardStatus(run)` collapses the run's latest entry into a colored status pill (`StatusPill`): `Waiting for you` (FORM present, amber), `Running` (IN_PROGRESS, sky+spinner), `Plan ready` (COMPLETED, emerald), `Needs attention` (FAILED/ERROR/HALTED, rose), plus the existing outbound `Sending…` / `Failed` / `Sent · waiting` states. Driven by `source.workflowStatus` + `source.hasForm` carried through the fan-out row. ✅ Phase 4 — extractor reads `user-answered-planner-form` rows as a third thread-entry class (`✓`), derives `run.pendingForm` (newest unanswered clarifying-questions FORM), and `deriveCardStatus` returns `Waiting for you` while it's set; `source.formQuestions` carries the questions. | The rendering primitives — the card's visual idiom is established; remaining phases extend it without rewriting. |
| `src/app/org/[githubLogin]/_components/SidebarChat.tsx` | ✅ Filter `message.source?.kind === "planner"` AND `"user-answered-planner-form"` rows out of the main scroll. ✅ Phase 4 — renders `<PlannerFormSlot>` beside each `SubAgentRunCard` whose `run.pendingForm` is set. Card-per-feature placement: anchor advances to the most recent activity message for each feature (extractor side). | The main message-render loop and proposal-card render arm. |
| `src/app/org/[githubLogin]/_components/PlannerFormSlot.tsx` | ✅ **New (Phase 4).** ~120 lines. Renders the run's most recent unanswered clarifying-questions FORM (`run.pendingForm`) as an inline answer-this-now card just outside the collapsed `SubAgentRunCard`, reusing `ClarifyingQuestionsPreview` verbatim with the `source.formQuestions` embedded on the fan-out row (no extra fetch). On submit, POSTs to `POST /api/orgs/[githubLogin]/planner-forms/answer`; tracks a **local** `submitted` flag to hide the FORM immediately (the persistent `✓ Answered` entry materializes on next conversation load — accepted staleness, open question #3). | — |
...
> - **The Phase-3 `SubAgentRunCard` status pill** (`Running` / `Waiting for you` / `Plan ready` / `Needs attention`) **was** added: the fan-out row now carries `source.workflowStatus` + `source.hasForm` (both optional, populated from the feature's live status and FORM-artifact presence at fan-out time), `CanvasMessageSource`'s `planner` variant gained those two optional fields, and `deriveCardStatus(run)` + `StatusPill` derive a colored pill from the run's latest entry. FORM presence outranks workflow status (a FORM is the "a human must pick" signal). Legacy rows without the new signals fall back to the Phase-2 `Replied` headline.
...
## Phase 4 — `PlannerFormSlot` (FORMs render inline, user answers in canvas chat) ✅ SHIPPED
...
> **Phase 4 implementation notes (post-ship).** Deviations from the plan as written:
> - **FORM representation correction (also retroactively fixes Phase 3).** Feature planners emit clarifying questions as a `PLAN` artifact with `isClarifyingQuestions(content)`, NOT `ArtifactType.FORM`. A shared `extractClarifyingQuestions(plannerMessage)` helper (`canvas-planner-fanout.ts`) is now the single detection site — used by `actionableWakeReason` (the `"form"` wake reason) AND by the fan-out row population (`hasForm` + `formQuestions`). The Phase 3 `ArtifactType.FORM` checks were replaced.
> - **Questions are embedded in the fan-out row** (`source.formQuestions: ClarifyingQuestion[]`), not fetched on demand and not registered via a separate `artifactIds[]` + `state.artifacts` store (the plan floated that). Embedding keeps the conversation self-contained — `PlannerFormSlot` renders with no extra round-trip, and the data round-trips through share / fork / iOS like every other message field. `CanvasMessageSource.planner` gained optional `hasForm` + `formQuestions`.
> - **Single-writer for the canvas conversation, not the optimistic-local-append the plan implied.** The endpoint is the sole writer of the `user-answered-planner-form` row (transaction-protected, idempotent on `plannerMessageId`); `PlannerFormSlot` only flips a local `submitted` flag to hide the FORM. This avoids a double-append race against the autosave PUT. The cost is the accepted client-staleness in open question #3 (the persistent `✓ Answered` thread entry appears on next load).
> - **Endpoint is idempotent + surfaces the planner's "already running" guard as a `409`** so the UI can show it. It calls `sendFeatureChatMessage` directly (with `replyId = plannerMessageId`) rather than re-POSTing to `/api/features/[id]/chat`.
> - **`pendingForm` is derived on the run** (`getSubAgentRunsFromMessages`): the newest inbound entry carrying `formQuestions` whose `plannerMessageId` has no matching `user-answered-planner-form` row. `deriveCardStatus` returns `Waiting for you` whenever `pendingForm` is set; the thread renders the answer as a third entry class (`✓`).
...
3. **[Phase 2] Race: fan-out vs autosave PUT (client-side staleness).** ✅ **RESOLVED — real-time push shipped.** Every server-side append (planner fan-out, autonomous canvas-agent turn, planner-form answer) now fires a `CANVAS_CONVERSATION_UPDATED` Pusher nudge on a per-conversation channel (`canvas-conversation-<id>`). The org canvas chat subscribes to its active conversation (in `useCanvasChatAutoSave`) and, on a nudge, refetches the conversation and replaces its local message list with the server's authoritative copy — so a user **sitting on the page sees planner messages and the agent's responses immediately**, no refresh. The replace runs only when the conversation is *idle* (no unsaved local messages, not streaming, no save in flight), coordinating with the autosave's `savedCount` so synced rows are never re-PUT as duplicates; a nudge that lands mid-turn is deferred and retried after the next save. (Original recommendation was to punt this; we brought it forward because live updates are a hard requirement.)
...
- ~~**Pusher push (optional optimization).**~~ ✅ **SHIPPED.** A per-conversation `CANVAS_CONVERSATION_UPDATED` broadcast fires on every server-side append; the canvas chat subscribes and refetches. See `notifyCanvasConversationUpdated` in `src/lib/pusher.ts`, the broadcast call sites (fan-out / auto-turn / planner-form answer), and the live-sync in `src/app/org/[githubLogin]/_state/useCanvasChatAutoSave.ts`.

- **Auto-turn runs off the webhook critical path.** The autonomous canvas-agent turn (`invokeCanvasAgentOnPlannerMessage`) is scheduled via Next.js `after()` in the Stakwork webhook route (`src/app/api/chat/response/route.ts`) — `fanOutPlannerMessageToCanvas` now just appends + pushes + *returns the wake reason*, and the route schedules the LLM turn after responding. This keeps the webhook ack to Stakwork fast (no 10–30s block) and avoids holding the request open for the turn. The whole planner ↔ canvas-agent loop is backend-only/event-driven (no browser required); the browser is a live reader via the Pusher nudge.
