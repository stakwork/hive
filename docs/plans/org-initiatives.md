...
3. **Agent annotation tools** (read-only over structure, write over annotation):
...
4. **"Promote a note to a Milestone"** — a button on an authored note that opens the milestone create-dialog with the note text pre-filled. The roadmap-is-the-product pattern's missing primitive.

5. **Workspace rollup** — workspace cards on the root canvas could carry an aggregate "N initiatives in flight" footer. Same pattern as the milestone-count footer on initiatives.

6. **Parent-canvas refresh on child edits** — when a milestone changes, the root initiative card's rollup needs refreshing. The Pusher fan-out (root + initiative) handles this; verify in practice.

7. **Live-node edit drawer** — double-clicking an initiative or milestone opens an edit drawer (or routes to the table UI's edit dialog). Not wired yet.
