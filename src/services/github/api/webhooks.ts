...
export async function createRepoHook(params: {
  ...
  if (!res.ok) {
    throw new Error(`Failed to create webhook: ${res.status} ${JSON.stringify(data)}`);
  }
...
export async function deleteRepoHook(token: string, owner: string, repo: string, hookId: number): Promise<void> {
