...
export const createSimulationStore = (dataStore: ReturnType<typeof createDataStore>, graphStore: any) =>
  create<SimulationStore>((set, get) => ({
    ...
    simulationCreate: (nodes) => {
      ...
      const simulation = forceSimulation(structuredClone(nodes)).numDimensions(3).stop();
    ...
    addNodesAndLinks: (newNodes, newLinks, replace) => {
      ...
      const validLinks = nextLinks.filter((l) => {
        ...
        return nextNodes.some((n) => n.ref_id === sourceId) && nextNodes.some((n) => n.ref_id === targetId);
      });
      ...
      simulation.force(
        "link",
        forceLink(validLinks).id((d: Node) => d.ref_id),
      );
    ...
    addLinkForce: () => {
      ...
      simulation
        ...
        .force(
          "charge",
          forceManyBody().strength((_d: NodeExtended) => -4),
        )
    ...
    addSplitForce: () => {
      ...
      simulation.force("charge", null).force("center", null).force("collide", null);
