...
/**
 * Variation B — Sparkles + shimmer + "Generate Plan"
 *
 * The "Generate Plan" pill is visually distinguished from the rest:
 * • Sparkles icon (AI/generation signal)
 * • Slight accent colour treatment (primary tint border + text) at rest
 * • CSS shimmer sweep animation across the pill to signal "AI action"
 * • On hover: fills with primary colour, icon pulses
 *
 * The other pills remain muted so this one stands out as the CTA.
 */

import { useState, useRef } from "react";
import { Send, X, Eye, Share2, Image as ImageIcon, Zap, Sparkles } from "lucide-react";
...
export default function PlanButtonB() {
      ...
      <div className="pointer-events-none flex flex-col justify-end max-h-[85vh]">
        ...
        {hasMessages && (
          <div className="pointer-events-auto flex items-center justify-end gap-1.5 px-1 pt-2 pb-1.5">
            ...
            <button className="plan-shimmer group relative flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border border-primary/30 text-primary font-medium hover:text-primary-foreground hover:border-primary transition-colors overflow-hidden backdrop-blur-sm">
              <Sparkles className="w-3 h-3 transition-transform group-hover:scale-110 group-hover:rotate-12" />
              Generate Plan
            </button>
