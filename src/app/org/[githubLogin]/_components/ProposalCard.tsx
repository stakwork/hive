...
export function ProposalCard({
  ...
  // codeChange: the diff is generated by a detached strut run — Approve
  // waits for it while the preview is pending and goes away once it failed
  // or was stopped (the card says why; re-generating is one message).
  const previewState =
    proposal.kind === "codeChange"
      ? getCodeChangePreviewState(proposal.payload)
      : "ready";
  ...
  const handleApprove = async () => {
    if (!activeId || !isPending || !previewReady) return;
 ...
 * Preview lifecycle (strut path): pending (spinner + link to the run),
 * failed / cancelled (the reason, no Approve), ready (the diff view).
...
function CodeChangeMeta({
  ...
  const previewState = getCodeChangePreviewState(payload);
  ...
  return (
    <div className="mt-0.5 space-y-1">
      ...
      {/* Preview pending — the strut run's completion patches the stored
          tool output in place and a live-sync nudge flips this state. */}
      ...
      {previewState === "failed" && (
        <div className="mt-1 text-[11px] text-rose-600 dark:text-rose-400">
          Failed: {payload.failure ?? "the diff could not be generated."}
        </div>
      )}
      {previewState === "cancelled" && (
        <div className="mt-1 text-[11px] text-muted-foreground">
          Stopped: {payload.failure ?? "the preview was cancelled."}
        </div>
      )}
