...
export default function TaskChatPage() {
  ...
  const handleNewMessage = useCallback((message: ChatMessage) => {
    if (taskMode === "agent" && !hasReceivedContentRef.current) {
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        // Agent mode: Hive owns the remote stream and webhooks persist output.
        if (taskMode === "agent") {
          ...
          const { podUrls } = await sessionResponse.json();
          ...
          // Webhook-persisted Pusher messages clear loading and update the UI.
    ...
    [taskMode, currentTaskId, clearLogs],
  );
