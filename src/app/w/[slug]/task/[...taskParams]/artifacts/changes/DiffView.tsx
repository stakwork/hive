...
function parseAndFormat(raw: string | object | null): string {
  ...
  try {
    ...
    if (typeof data === "string") {
      // Couldn't parse to an object — return as-is (plain text)
      return raw;
    }

    return JSON.stringify(omitNoiseFields(data), null, 2);
  } catch (e) {
    console.error("DiffView: Failed to parse JSON:", e);
    return raw;
  }
}
              ...
              ? contextLines.map((entry: FlatLine, index: number) => {
                  ...
                  return (
                    <tr key={`flat-${index}`} className={bgColor}>
                      <td
                        className={`w-8 px-2 py-0.5 text-right select-none border-r border-border/50 ${textColor || "text-muted-foreground"}`}
                      >
                        {Icon ? (
                  ...
                  return lines.map((line: string, lineIndex: number) => {
                    ...
                    return (
                      <tr key={`${index}-${lineIndex}`} className={bgColor}>
                        ...
                        <td
                          className={`px-3 py-0.5 whitespace-pre ${textColor || "text-muted-foreground/50"}`}
                        >
                          {line || " "}
                        </td>
                      </tr>
