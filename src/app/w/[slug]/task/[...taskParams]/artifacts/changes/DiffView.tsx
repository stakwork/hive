...
export interface DiffViewProps {
  ...
  updated: string | null;
  /** Optional label used in empty-state messages, e.g. "workflow", "prompt", "script" */
  label?: string;
}
...
function parseAndFormatJson(jsonString: string): string {
  try {
...
function normalizeContent(content: string | null): string {
  if (!content) return "";
  if (looksLikeJson(content)) return parseAndFormatJson(content);
  return content;
}

function buildDiffData(original: string | null, updated: string | null) {
  const normalizedOriginal = normalizeContent(original);
  const normalizedUpdated = normalizeContent(updated);

  if (!normalizedOriginal && !normalizedUpdated) {
    return { changes: [] as DiffPart[], contextLines: [] as FlatLine[], stats: { additions: 0, deletions: 0 } };
  }

  const diff = diffLines(normalizedOriginal, normalizedUpdated) as DiffPart[];
...
export function DiffView({ original, updated, label = "content" }: DiffViewProps) {
  ...
  const { changes, contextLines, stats } = useMemo(
    () => buildDiffData(original, updated),
    [original, updated],
  );
    ...
    return (
      <div className="flex items-center justify-center h-full p-8">
        <div className="text-muted-foreground text-sm">
          No {label} data available for comparison
        </div>
      </div>
    );
  }
  ...
  if (!updated) {
    return (
      <div className="flex items-center justify-center h-full p-8">
        <div className="text-muted-foreground text-sm">No updated {label} available</div>
      </div>
    );
  }
  ...
  const isJsonContent = original ? looksLikeJson(original) : updated ? looksLikeJson(updated) : false;
  const fullToggleLabel = isJsonContent ? "Full JSON" : "Full content";
  ...
  return (
    <div className={`h-full flex flex-col ${isDark ? "dark" : ""}`}>
      ...
      {/* Diff content */}
      <div className="flex-1 overflow-auto font-mono text-xs" style={hideScrollbarStyle}>
        <style>{`
              ...
              ? contextLines.map((entry: FlatLine, index: number) => {
                  if (entry.type === "separator") {
                    return (
                      <tr key={`sep-${index}`}>
                        <td
                          colSpan={2}
                          className="text-muted-foreground text-center py-0.5 text-xs select-none border-y border-border/30"
                        >
                          ...
                        </td>
                      </tr>
                  ...
                  return (
                    <tr key={`flat-${index}`} className={bgColor}>
                      <td
                        className={`w-8 px-2 py-0.5 text-right select-none border-r border-border/50 ${
