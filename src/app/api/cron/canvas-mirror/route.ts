...
import { runCanvasMirror } from "@/services/canvas-mirror-cron";

// Allow up to 5 minutes; one pass is bounded by maxPerType per org.
...
export async function GET(request: NextRequest) {
  ...
  logger.info(
    `[CANVAS MIRROR] cron invoked (depth=${depth})`,
    "CANVAS_MIRROR",
  );

  const result = await runCanvasMirror();
  ...
  if (result.anyCapped && depth < MAX_CHAIN_DEPTH) {
    ...
    after(async () => {
      try {
        ...
        logger.warn("[CANVAS MIRROR] self-chain trigger failed", "CANVAS_MIRROR", { error });
      }
    });
  }
