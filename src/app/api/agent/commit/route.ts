...
export async function POST(request: NextRequest) {
  try {
    ...
    if (workspace.sourceControlOrg) {
      logger.debug(">>> Getting user app token for workspace source control org:", { githubUsername: userGithubAuth?.githubUsername, orgLogin: workspace.sourceControlOrg.githubLogin });
    ...
    logger.debug(">>> Push URL:", { pushUrl, mode: existingPullRequest ? "staying on current branch" : "creating new branch" });
