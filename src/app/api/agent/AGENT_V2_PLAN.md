...
### 2. `src/types/streaming.ts`

Update types to align with AI SDK's `TextStreamPart`:

**Changes:**
- `TextDeltaEvent`: Change `delta` to `text`
- `ReasoningDeltaEvent`: Change `delta` to `text`
- Add `ToolCallEvent`, `ToolResultEvent`, `ToolErrorEvent`
- Add `FinishEvent`, `StartEvent`
...
export interface TextDeltaEvent extends BaseStreamEvent {
  type: "text-delta";
  id: string;
  text: string;  // Changed from 'delta' to 'text'
}
...
### 3. `src/lib/streaming/useStreamProcessor.ts`

Update to handle AI SDK native format:

**Key changes:**
```typescript
// Line ~163: text-delta uses 'text' not 'delta'
} else if (data.type === "text-delta") {
  const uniqueId = streamIdToUniqueId.get(data.id);
  if (uniqueId) {
    textParts.set(uniqueId, (textParts.get(uniqueId) || "") + data.text);  // Changed from data.delta
  }
}

// Line ~176: reasoning-delta uses 'text' not 'delta'  
} else if (data.type === "reasoning-delta") {
  reasoningParts.set(data.id, (reasoningParts.get(data.id) || "") + data.text);  // Changed from data.delta
}

// Add handler for tool-call (AI SDK native format)
...
// Add handler for tool-result (AI SDK native format)
...
### 4. `src/app/w/[slug]/task/[...taskParams]/page.tsx`
...
AI SDK STREAMING MESSAGE TYPE FOR REFERENCE:
