...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!uri) {
      return NextResponse.json({ success: false, message: "Provide url please" }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: false, message: "Failed to validate uri" }, { status: 500 });
  }
}
