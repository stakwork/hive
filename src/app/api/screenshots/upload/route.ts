...
export async function POST(request: NextRequest) {
  try {
    ...
    const { dataUrl, workspaceId, taskId, actionIndex, pageUrl, timestamp, width, height } = validatedData;
    ...
    if (taskId) {
      ...
      if (!task) {
        return NextResponse.json({ error: "Task not found or does not belong to workspace" }, { status: 404 });
      }
    }
