...
export const CapacityVisualization3D = forwardRef<CapacityVisualization3DRef, CapacityVisualization3DProps>(({ vmData, onServerClick }, ref) => {
    ...
    const handleRecenter = () => {
        ...
        cameraControlsRef.current?.setLookAt(
            0, 5, 12, // Position: Centered, slightly up and back
            0, 2, 0,  // Target: Center of the aisle
    ...
    return (
        <div className="w-full h-[calc(100vh-12rem)] bg-black rounded-xl overflow-hidden border border-white/10 relative">
            <Canvas shadows camera={{ position: [0, 5, 12], fov: 45 }}>
                ...
                <rectAreaLight width={20} height={2} color="#ffffff" intensity={2} position={[0, 10, 0]} rotation={[-Math.PI / 2, 0, 0]} />
                <rectAreaLight width={20} height={2} color="#ffffff" intensity={2} position={[0, 10, 10]} rotation={[-Math.PI / 2, 0, 0]} />
                <rectAreaLight width={20} height={2} color="#ffffff" intensity={2} position={[0, 10, -10]} rotation={[-Math.PI / 2, 0, 0]} />
