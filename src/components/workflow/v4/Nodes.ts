...
class Nodes {
  ...
  static ifConditionNode({
    ...
    if(status){
      questionFill = icon.replace('fill="#444851"', 'fill="#FFFFFF"');
    } else {
      questionFill = icon;
    }
    if(show_only === false || show_only === 'false'){
      dataElem = `<div class="workflow-ifcon-inner"><span class="workflow-ifcon-icon">${questionFill}</span>IfCondition<div class="workflow-ifconText">${data}</div><div class="workflow-ifcon-edit step-menu-edit" data-unique-id="${uniqueId}" data-wizard-step="true"><i class="material-icons">edit</i></div></div>`;
    }else{
      // In show_only mode (e.g., editor mode), make the node clickable for step selection
      // without using anchor tags that would navigate to non-existent routes
      dataElem = `<div class="workflow-ifcon-inner workflow-show-modal" data-unique-id="${uniqueId}"><span class="workflow-ifcon-icon">${questionFill}</span>IfCondition<div class="workflow-ifconText">${data}</div></div>`;
    }
