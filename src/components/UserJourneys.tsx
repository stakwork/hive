...
export default function UserJourneys() {
  ...
  const fetchUserJourneyTasks = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        logger.error("Failed to fetch user journey tasks");
        return;
      }
      ...
      logger.error("Error fetching user journey tasks:", { error });
    } finally {
  ...
  const fetchE2eTestsFromGraph = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        logger.error("Failed to fetch E2E tests from graph");
        return;
      }
      ...
      logger.error("Error fetching E2E tests from graph:", { error });
    }
  }, [slug]);
  ...
  const fetchTestCode = async (task: UserJourneyTask): Promise<string | null> => {
    try {
      ...
      logger.error("Error fetching test code:", { error });
      return null;
    }
  };
  ...
  const handleCreateUserJourney = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error("Failed to claim pod:", { errorData });
  ...
  const handleReplay = async (task: UserJourneyTask) => {
    ...
    try {
      ...
      logger.error("Error starting replay:", { error });
      toast({
        variant: "destructive",
        title: "Replay Error",
  ...
  const saveUserJourneyTest = async (filename: string, generatedCode: string) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        logger.error("Failed to save user journey:", { errorData });
        toast({
          variant: "destructive",
      ...
      logger.error("Error saving user journey:", { error });
      toast({
        variant: "destructive",
        title: "Error",
        description: "An unexpected error occurred. Please try again.",
