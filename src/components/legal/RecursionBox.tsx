...
function RecursionCard({ entry, refetch }: RecursionCardProps) {
  ...
  // `mapSecondary` tags it as "recursion" runType — so we match on the
  // raw StakworkRunType by checking `taskSlug` and status.
  const existingConsolidated = useMemo(() => {
    return allRuns
      .filter(
        (r) =>
          r.taskSlug === entry.id &&
          // mapSecondary doesn't tag the run type directly; we rely on the
          // fact that CONSOLIDATED rows appear in allRuns after Step 3
          // of the feature. We match any "recursion" tagged row that is
          // PENDING or IN_PROGRESS (no hasReport) as a proxy for in-flight
          // consolidated runs, to prevent double-dispatch.
          r.runType === "recursion" &&
