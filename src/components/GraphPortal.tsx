      ...
      try {
        const results = await Promise.all(
          workspaces.map(async (ws) => {
            ...
            const repositories: RepoSummary[] = rawRepos.map(r => ({
              id: r.id,
              name: r.name,
...
function createRepoCodeLoader(slug: string, prefix: string, repoId: string, repositoryUrl: string): NodeLoader {
  return async () => {
    ...
    let url = `/api/workspaces/${slug}/graph/nodes?node_type=${encodeURIComponent(nodeTypes)}&limit=100&limit_mode=per_type`;
    if (repositoryUrl) {
      url += `&repo_url=${encodeURIComponent(repositoryUrl)}`;
    }
    ...
    const nodeIdFor = (refId: string) =>
      repoId ? `${prefix}repo-${repoId}-${refId}` : `${prefix}repo-${refId}`;
    const typeGroupIdFor = (nodeType: string) =>
      repoId ? `${prefix}repo-${repoId}-type-${nodeType}` : `${prefix}repo-type-${nodeType}`;

    const nodes: RawNode[] = [];
    const edges: RawEdge[] = [];

    for (const [nodeType, groupNodes] of typeGroups) {
      const typeGroupId = typeGroupIdFor(nodeType);
      ...
      for (const n of groupNodes) {
        ...
        nodes.push({ id: nodeIdFor(n.ref_id as string), label: truncLabel(name), content: n.node_type as string });
        edges.push({ source: typeGroupId, target: nodeIdFor(n.ref_id as string) });
      }
    }
    ...
    for (const e of rawEdges) {
      if (refIdSet.has(e.source as string) && refIdSet.has(e.target as string)) {
        edges.push({ source: nodeIdFor(e.source as string), target: nodeIdFor(e.target as string) });
      }
    }
...
export function createRepoPRLoader(tasks: TaskSummary[], slug: string, prefix: string, repoId: string, repositoryUrl: string, fromPath?: string): NodeLoader {
  return async () => {
    const nodes: RawNode[] = [];
    const edges: RawEdge[] = [];
...
export function buildWorkspaceGraph(
  ...
  for (const ws of allData) {
    ...
    // Repository — loadable (per-repo nodes when repos are configured, fallback otherwise)
    if (ws.repositories.length > 0) {
      ...
      // Fallback: original single-repo structure
...
export function GraphPortal({ workspaces: externalWorkspaces, embedded }: GraphPortalProps = {}) {
  ...
  const loaders = useMemo(() => {
    ...
    for (const ws of allWsData) {
      ...
      if (ws.repositories.length > 0) {
        ...
        // Fallback loaders for workspaces with no configured repos
  ...
  // Search effect: highlight nodes whose label matches searchText
  useEffect(() => {
    if (!graph || !searchText.trim()) { setQueryMatches(null); return; }
        ...
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
          ...
          <div style={{
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4dd9e8"
              strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.6 }}>
