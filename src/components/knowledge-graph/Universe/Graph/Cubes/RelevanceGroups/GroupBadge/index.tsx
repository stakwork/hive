...
export const GroupBadge = ({ position, name, count, onClick, isActive }: BadgeProps) => (
  <group position={position}>
    <Html center distanceFactor={250} sprite transform zIndexRange={[0, 0]}>
      <div
        className={`relative text-white bg-black p-3 rounded-lg border transition-all duration-200 cursor-pointer ${isActive ? 'border-blue-500 bg-blue-900/30' : 'border-gray-700 hover:border-gray-600'
          }`}
        onClick={(e) => {
          e.stopPropagation()
          onClick()
      ...
      >
        {name ? (
          <span className="text-sm font-medium">{truncateText(name, 15)}</span>
        ) : null}
        <div
