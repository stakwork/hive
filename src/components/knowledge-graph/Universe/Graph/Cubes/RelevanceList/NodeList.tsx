  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-gray-700">
        <h3 className="text-lg font-semibold text-white mb-2">Related Nodes</h3>
        {selectedNodeType ? (
          <div className="text-sm text-gray-400">
            Showing {selectedNodeType} nodes ({childNodes.length})
          </div>
        ) : (
          <div className="text-sm text-gray-400">All connected nodes ({childNodes.length})</div>
        )}
      </div>
        ...
        {childNodes.length > 0 ? (
          <div className="space-y-3">
            {childNodes.slice(0, 20).map((node: NodeExtended, index: number) => (
              <NodeCard key={node.ref_id || index} node={node} compact onClick={handleNodeClick} />
            ))}
          ...
          <div className="text-sm text-gray-400 text-center py-8">No related {selectedNodeType} nodes found</div>
        ) : (
          <div className="text-sm text-gray-400 text-center py-8">No connected nodes found</div>
        )}
      </div>
      ...
      {childNodes.length > 0 && (
        <div className="p-3 border-t border-gray-700 bg-gray-900/30">
          <div className="text-xs text-gray-400 text-center">Click any node to navigate to it</div>
        </div>
      )}
    </div>
  );
