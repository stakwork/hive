...
import { ensureBifrostTrust, reconcileBifrostVK } from "@/services/bifrost";
...
export async function maybeReconcileBifrost(
  ...
  // Phase-5 trust reconcile runs ahead of the VK reconcile: ensure
  // the workspace's org has a macaroon-signing keypair (autogenerated
  // on first use) and the plugin's trust registry knows about it.
  // Both steps are content-addressed; on the hot path they're a
  // single DB read each. Failure here is logged inside
  // `ensureBifrostTrust` and is non-fatal — VK reconcile still runs
  // and LLM calls still work, since macaroon enforcement is off
  // through phase 5. See `gateway/plans/phases/phase-5-trust-registry.md`.
  try {
    const trustResult = await ensureBifrostTrust(workspaceAuth.workspaceId);
    if (trustResult.status === "failed") {
      // Already logged by ensureBifrostTrust; we deliberately fall
      // through to VK reconcile so a transient plugin issue doesn't
      // block LLM calls.
    }
  } catch (err) {
    // Defensive — ensureBifrostTrust catches its own errors and
    // returns `failed`; this is for unexpected throws (e.g. lock
    // acquire timeout, DB connection drop).
    logger.warn(
