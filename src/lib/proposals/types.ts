...
export interface CodeChangeProposalPayload {
  ...
  /**
   * Preview lifecycle when the diff is generated by a detached strut run
   * (`dispatchStrutRun`, kind `code_change_propose`): the tool returns the
   * card `pending` with an empty diff, and the run's completion handler
   * patches the stored tool output in place to `ready` (diff filled in) or
   * `failed` / `cancelled`. Absent on proposals from the synchronous path,
   * which are always ready — read it through `getCodeChangePreviewState`.
   */
  preview?: CodeChangePreviewState;
  /** Set while `preview === "pending"`: where the diff is being generated. */
  pending?: CodeChangePendingPreview;
  /** Set when `preview` is `failed` / `cancelled`: why, for the card. */
  failure?: string;
}

export type CodeChangePreviewState = "pending" | "ready" | "failed" | "cancelled";

export interface CodeChangePendingPreview {
  /** Hive's `StrutRun.id` — the active-run key. */
  runId: string;
...
export function getCodeChangePreviewState(
  payload: Pick<CodeChangeProposalPayload, "preview">,
): CodeChangePreviewState {
  return payload.preview ?? "ready";
}
...
/**
 * The `StrutRun.kind` / strut workflow behind a code-change PREVIEW
 * (`services/strut-runs/code-change-propose.ts`). Declared here — a
 * dependency-free module — so the approval handler can look the row up
 * without importing the completion handler's delivery graph.
 */
