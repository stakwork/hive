    ...
    try {
      ...
      if (frontendService?.port) {
        ...
        if (frontend) {
          ...
          logger.debug(`>>> Port ${frontendService.port} from services not found in port mappings, trying fallbacks`, "pods/utils");
        }
      } else {
        console.log(">>> No frontend service found in services array, trying fallbacks");
      }
    } catch (error) {
      logger.error(">>> Error getting port from services array, trying fallbacks:", "pods/utils", { error });
    }
  }
