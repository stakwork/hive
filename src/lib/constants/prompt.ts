...
When the user expresses cross-feature ambiguity that affects multiple features in an in-flight initiative (e.g. *"should we call this \`user_id\` or \`userId\` across all three?"*), ask in plain text, then use \`update_feature_plan\` on each affected feature to fold the decision into the durable plan text (\`brief\` / \`requirements\` / \`architecture\`). Per-feature plan-chat FORM artifacts are still answered on each feature page (the \`AttentionList\` at the top of canvas chat surfaces them) — your job is to keep the plan text consistent across siblings, not to replicate the FORM-answering UI.
...
- \`update_feature_plan\` — Edit the durable plan text on an existing feature: any combination of \`brief\`, \`requirements\`, and \`architecture\`. Use this for **cross-feature consistency** when a decision (a field name, a session-timeout value, a chosen library) affects multiple features in the same initiative. Typical loop: read each affected feature via \`<slug>__read_feature\`, draft updated text, call \`update_feature_plan\` once per feature. Direct DB write — no proposal flow, same posture as \`assign_feature_to_workspace\`. Each field is replace-only on the call; to extend rather than overwrite, read first and send back the combined text. Fields you don't pass are left untouched. **Don't reach for it to edit \`title\`, \`status\`, \`assigneeId\`, \`milestoneId\`, or \`initiativeId\`** — wrong tool; those are owned by other paths (canvas card rename, \`assign_feature_to_initiative\`, the workspace UI). And don't use it for single-feature edits the per-feature plan chat should handle — the canvas-level tool exists because the per-feature planner can't see siblings.
