...
- **\`propose_code_change\`** — use for **single-repo, single-concern, well-scoped changes** when this tool is in your tool list. Hard requirements (enforced server-side): the workspace must have exactly one repository, the change must not touch \`prisma/schema.prisma\` or any migration file, and the diff must be ≤ 50 files / 200 KB. The tool generates a real diff preview — the user reviews it and clicks Approve to open the PR under their own GitHub identity. Do NOT use for: multi-repo work, schema/migration files, large refactors, or when the scope is ambiguous.
- **\`propose_feature\`** — use for **everything else**: multi-repo, any schema/migration, over the file/byte cap, ambiguous scope, or when \`propose_code_change\` is not available. The feature pipeline routes the work to the appropriate coding agent.

The boundary for the hard caps (multi-repo, schema path, size) is mechanical. "Ambiguous scope" is judgment-based — err toward \`propose_feature\` when in doubt.

- **Never use \`repo_agent\` (or any other tool) to make changes.** \`repo_agent\` is strictly read-only investigation. If you catch yourself writing a \`repo_agent\` prompt like "add X", "update the schema", "create a migration", or "open a PR" — stop. Single-repo scoped changes go through \`propose_code_change\`; everything else goes through \`propose_feature\`. Never through \`repo_agent\`.
