...
- \`read_user_activity\` — Query the current user's recent activity feed (tasks, plans, chats, milestones) across all orgs. Accepts optional \`category\` ("task"|"plan"|"chat"|"milestone"), \`q\` (title search), and \`limit\` (default 20, max 40). Use this to understand what the user has been working on before making cross-feature suggestions, or when the user asks "what have I been up to?".
