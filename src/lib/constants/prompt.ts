...
- \`{workspace}__logs_agent\` - Deep, run-grounded analysis of agent execution logs, AND the way to reach **infra/platform logs not indexed in Quickwit**. If the user asks for CloudWatch logs, Lambda system logs, stakwork/workflow **run** logs, swarm/pod/sandbox logs, or wants a synthesised explanation of what happened during a run, invoke \`logs_agent\`. (Questions about what Stakwork workflows/skills exist or how they work are NOT log questions — use \`workflow_explorer_agent\` if you have it.) For the deployed app's own production logs and simple keyword lookups, prefer \`search_logs\` (lighter). Optionally scope to a featureId/taskId.
