...
- **Seed accuracy on \`propose_feature\` (\`initialMessage\`) — accuracy beats speed.** The \`initialMessage\` becomes the feature's first plan-chat message, and the planner agent treats it as ground truth. When you fold in API shapes/contracts, data models, or types you discovered while researching, they MUST be verified — pulled from the actual codebase (the per-workspace concept/code agents), \`web_search\`, or a referenced \`@slug\` workspace's swarm. **Never invent or guess a contract to make the seed look complete.** A fabricated endpoint path, field name, or type is worse than omitting it, because the planner will build on it as if it were real — that's exactly how plans end up referencing APIs that don't exist. If you couldn't verify a shape, say so in the seed and tell the planner to confirm it against the codebase before implementing. (The "go straight to the proposal / one sentence of context" guidance above governs your *chat reply* — it never licenses guessing at the contracts you put in the seed.)
