...
// Lazy-load encryption service to avoid importing Node crypto at module load time
// This is important for Edge Runtime compatibility in middleware
...
export const authConfig: NextAuthConfig = {
  ...
  ...(process.env.POD_URL ? {} : { adapter: PrismaAdapter(db) as any }),
        ...
        try {
          ...
          if (existingUser) {
            ...
            if (!existingAccount) {
              ...
              const encryptedAccessToken = getEncryptionService().encryptField("access_token", account.access_token ?? "");
              ...
              await db.account.create({
                data: {
                  ...
                  refresh_token: account.refresh_token
                    ? JSON.stringify(getEncryptionService().encryptField("refresh_token", account.refresh_token))
                  ...
                  id_token: account.id_token
                    ? JSON.stringify(getEncryptionService().encryptField("id_token", account.id_token))
                    : (null as unknown as string | undefined | null),
          ...
          if (account && account.access_token) {
            try {
              ...
              const { data: githubProfile } = await axios.get<GitHubProfile>("https://api.github.com/user", {
                headers: {
                  Authorization: `token ${getEncryptionService().decryptField("access_token", account.access_token)}`,
                },
              });
    ...
    async linkAccount({ user, account }) {
      try {
        if (account?.provider === "github" && account.access_token) {
          const encryptedToken = JSON.stringify(getEncryptionService().encryptField("access_token", account.access_token));
