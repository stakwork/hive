...
import {
  ...
  createBearerPatchRequest,
} from "@/__tests__/support/helpers";
import { createTestApiKey } from "@/__tests__/support/factories";
...
describe("PATCH /api/tickets/[ticketId] - Integration Tests", () => {
  ...
  describe("Bearer API Key Authentication", () => {
    test("valid hive_ key returns 200 and applies update using key creator identity", async () => {
      const owner = await createTestUser();
      const workspace = await createTestWorkspace({
        ownerId: owner.id,
        name: "Test Workspace",
        slug: "test-workspace-bearer",
      });
      ...
      const request = createBearerPatchRequest(
        `http://localhost:3000/api/tickets/${ticket.id}`,
        { title: "Bearer Updated Title" },
        rawKey
      );
      ...
      expect(updatedTask?.title).toBe("Bearer Updated Title");
    });
    ...
    test("revoked key returns 401", async () => {
      ...
      const workspace = await createTestWorkspace({
        ownerId: owner.id,
        name: "Test Workspace Revoked",
        slug: "test-workspace-revoked",
      });
    ...
    test("expired key returns 401", async () => {
      ...
      const workspace = await createTestWorkspace({
        ownerId: owner.id,
        name: "Test Workspace Expired",
        slug: "test-workspace-expired",
      });
    ...
    test("invalid token returns 401", async () => {
      ...
      const request = createBearerPatchRequest(
        `http://localhost:3000/api/tickets/${ticket.id}`,
        { title: "Should Not Update" },
        "hive_invalid_token_that_does_not_exist"
      );
