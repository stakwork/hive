...
describe("POST /api/learnings - Authorization", () => {
  ...
  beforeEach(async () => {
    await db.$transaction(async (tx) => {
      ...
      const encryptedApiKey = encryptionService.encryptField(
        "swarmApiKey",
        "test-post-swarm-api-key"
      );

      swarm = await createTestSwarm({
        workspaceId: workspace.id,
        name: `test-post-swarm-${generateUniqueId("swarm")}`,
        status: "ACTIVE",
      });
      ...
      await tx.swarm.update({
        where: { id: swarm.id },
        data: {
          swarmUrl: "https://test-post-swarm.sphinx.chat",
          swarmApiKey: JSON.stringify(encryptedApiKey),
        },
      });
  ...
  it("should allow VIEWER role to seed learnings", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      expect.stringContaining("https://test-post-swarm.sphinx.chat:3355/seed_stories?budget=10"),
      expect.objectContaining({
        method: "POST",
        headers: expect.objectContaining({
          "x-api-token": "test-post-swarm-api-key",
        }),
      })
    );
...
describe("POST /api/learnings - Data Integrity", () => {
  ...
  it("should implement fire-and-forget pattern (immediate response)", async () => {
  ...
  it("should construct correct URL with /seed_stories endpoint", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ success: true }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    expect(fetchUrl).toMatch(
      /^https:\/\/post-integrity-swarm\.sphinx\.chat:3355\/seed_stories\?budget=10$/
    );
  ...
  it("should encode budget parameter in URL", async () => {
    ...
    const budgetWithSpecialChars = "100&limit=50";
    const request = createAuthenticatedPostRequest(
      `/api/learnings?workspace=${workspace.slug}&budget=${encodeURIComponent(budgetWithSpecialChars)}`,
      {},
      owner
    );
    await POST(request);
    ...
    expect(fetchUrl).toContain(
      `budget=${encodeURIComponent(budgetWithSpecialChars)}`
    );
  ...
  it("should not block on external swarm server failures (fire-and-forget)", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("Swarm seed_stories error: 500")
    );

    consoleSpy.mockRestore();
  });

  it("should handle external API network errors gracefully (fire-and-forget)", async () => {
    const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
  ...
  it("should construct correct swarm URL with port 3355", async () => {
    ...
    expect(fetchUrl).toMatch(
      /^https:\/\/post-integrity-swarm\.sphinx\.chat:3355\/seed_stories/
    );
