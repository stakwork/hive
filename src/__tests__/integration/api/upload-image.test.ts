...
describe('POST /api/upload/image Integration Tests', () => {
  ...
  describe('Input Validation Tests', () => {
    ...
    test('should return 400 for negative file size', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));
  ...
  describe('File Security Tests', () => {
    ...
    test('should accept file exactly at 10MB limit', async () => {
      ...
      // Mock S3Service to accept file at limit
      vi.mocked(mockS3Service.validateFileType).mockReturnValue(true);
      vi.mocked(mockS3Service.validateFileSize).mockReturnValue(true);
      vi.mocked(mockS3Service.generatePresignedUploadUrl).mockResolvedValue(
        'https://test-bucket.s3.us-east-1.amazonaws.com/upload-url',
  ...
  describe('S3 Path Generation Tests', () => {
    test('should generate S3 path with correct hierarchy', async () => {
      ...
      // Verify S3 path follows pattern: features/{workspaceId}/{swarmId}/{featureId}/{timestamp}_{randomId}_{filename}
      // Note: The route uses swarm.id (database ID), not swarm.swarmId (external ID)
      expect(data.s3Path).toMatch(
        new RegExp(`^features/${testWorkspace.id}/${testSwarm.id}/${testFeature.id}/\\d+_[a-z0-9]+_test\\.jpg$`),
      );
    });
    ...
    test('should sanitize filename with special characters', async () => {
      ...
      expect(data.s3Path).toMatch(/file_with_spaces___special_chars_\.jpg$/);
    });
    ...
    test('should use default swarmId when swarm is not configured', async () => {
      ...
      // Verify default swarmId is used when workspace has no swarm
      expect(data.s3Path).toMatch(
        new RegExp(`^features/${testWorkspace.id}/default/${testFeature.id}/`),
      );
    });
  });

  describe('Presigned URL Properties Tests', () => {
    test('should generate presigned upload URL with 5-minute expiry', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));
      ...
      // Verify presigned upload URL generated with 5 minutes (300 seconds) expiry
    ...
    test('should generate presigned download URL with 1-year expiry', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));
      ...
      // Verify presigned download URL (publicUrl) generated with 1 year expiry
      expect(mockS3Service.generatePresignedDownloadUrl).toHaveBeenCalledWith(
        expect.stringMatching(/^features\//),
        604800, // 1 week in seconds (7 days)
      );
    });
  ...
  describe('Error Handling Tests', () => {
    test('should handle S3 presigned upload URL generation failure gracefully', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));

      // Mock S3Service to pass validation but fail on upload URL generation
      vi.mocked(mockS3Service.validateFileType).mockReturnValue(true);
      vi.mocked(mockS3Service.validateFileSize).mockReturnValue(true);
    ...
    test('should handle S3 presigned download URL generation failure gracefully', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));

      // Mock S3Service to pass validation and upload URL, but fail on download URL
      vi.mocked(mockS3Service.validateFileType).mockReturnValue(true);
      vi.mocked(mockS3Service.validateFileSize).mockReturnValue(true);
      vi.mocked(mockS3Service.generatePresignedUploadUrl).mockResolvedValue(
        'https://test-bucket.s3.us-east-1.amazonaws.com/upload-url',
      ...
      vi.mocked(mockS3Service.generatePresignedDownloadUrl).mockRejectedValue(
        new Error('S3 Error: Failed to generate download URL'),
      );
  ...
  describe('Edge Cases', () => {
    test('should handle very long filename', async () => {
      ...
      // Verify long filename is handled (may be truncated or sanitized)
      expect(data.s3Path).toBeDefined();
      expect(data.filename).toBe(longFilename);
    });
    ...
    test('should handle filename with spaces', async () => {
      ...
      const request = createPostRequest('http://localhost:3000/api/upload/image', {
        featureId: testFeature.id,
        filename: 'file with many spaces.jpg',
        contentType: 'image/jpeg',
        size: 1024000,
      });
      ...
      // Verify spaces are sanitized in S3 path
      expect(data.s3Path).toMatch(/file_with_many_spaces\.jpg$/);
    });

    test('should handle filename with unicode characters', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));
      ...
      const request = createPostRequest('http://localhost:3000/api/upload/image', {
        featureId: testFeature.id,
        filename: 'файл-с-юникодом.jpg', // Unicode filename
        contentType: 'image/jpeg',
        size: 1024000,
      });
      ...
      // Verify unicode characters are sanitized
      expect(data.s3Path).toMatch(/\.jpg$/);
    });
    ...
    test('should handle filename with multiple dots', async () => {
      ...
      // Verify filename with multiple dots is preserved
      expect(data.filename).toBe('file.test.backup.jpg');
      expect(data.s3Path).toMatch(/file\.test\.backup\.jpg$/);
    });

    test('should handle file size just under 10MB limit', async () => {
      const { testUser, testFeature } = await createTestUserWithWorkspaceAndFeature();
      getMockedSession().mockResolvedValue(createAuthenticatedSession(testUser));
      ...
      expect(response.status).toBe(200);
      expect(mockS3Service.validateFileSize).toHaveBeenCalledWith(10 * 1024 * 1024 - 1);
    });
  });
});
