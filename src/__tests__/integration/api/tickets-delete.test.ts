...
describe('DELETE /api/tasks/[taskId]', () => {
  ...
  describe('Authorization', () => {
    test('should return 401 if user is not authenticated', async () => {
      // Create request without user - no auth headers injected
  ...
  describe('Data Integrity - Orphaned Dependencies', () => {
    test('should clean up orphaned dependencies when task is deleted', async () => {
      ...
      // ✅ Verify orphaned dependency is cleaned up
    ...
    test('should clean up multiple orphaned dependencies', async () => {
      ...
      // Verify both dependent tasks have cleaned up orphaned references
    ...
    test('should clean up mixed dependencies correctly', async () => {
      ...
      // Create dependent with mixed dependencies (one to be deleted, one to remain)
      const dependentTask = await createTestTask({
      ...
      // Verify dependent only has valid reference remaining
      const updated = await findTestTask(dependentTask.id);
      expect(updated?.dependsOnTaskIds).not.toContain(task.id);  // Cleaned up
      expect(updated?.dependsOnTaskIds).toContain(anotherTask.id);  // Preserved
