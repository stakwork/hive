...
describe("Workspace Logo API Integration Tests", () => {
  ...
  describe("POST /api/workspaces/[slug]/settings/image/upload-url", () => {
    describe("Authentication Tests", () => {
      test("should return 401 for unauthenticated request", async () => {
        const request = createPostRequest("http://localhost:3000/api/workspaces/test/settings/image/upload-url", {
          filename: "logo.jpg",
    ...
    describe("Permission Tests", () => {
      test("should allow OWNER to upload logo", async () => {
        ...
        mockS3Service.generateWorkspaceLogoPath.mockReturnValue("workspace-logos/test/123.jpg");
        mockS3Service.generatePresignedUploadUrl.mockResolvedValue("https://s3.amazonaws.com/presigned-url");
      ...
      test("should allow ADMIN to upload logo", async () => {
        const { testUser, testWorkspace } = await createTestUserAndWorkspace(WorkspaceRole.ADMIN);
      ...
      test("should return 403 for VIEWER role", async () => {
        const { testUser, testWorkspace } = await createTestUserAndWorkspace(WorkspaceRole.VIEWER);
  ...
  describe("POST /api/workspaces/[slug]/settings/image/confirm", () => {
    describe("Authentication Tests", () => {
      test("should return 401 for unauthenticated request", async () => {
        const request = createPostRequest("http://localhost:3000/api/workspaces/test/settings/image/confirm", {
    ...
    describe("Image Processing Tests", () => {
      ...
      test("should delete old logo when uploading new one", async () => {
        ...
        expect(mockS3Service.deleteObject).toHaveBeenCalledWith("workspace-logos/test/old.jpg");
      });
      ...
      test("should reject corrupt image file", async () => {
        ...
        expect(response.status).toBe(400);
        expect(mockS3Service.deleteObject).toHaveBeenCalledWith("workspace-logos/test/123.jpg");
      });
    });
  });
  ...
  describe("GET /api/workspaces/[slug]/image", () => {
    ...
    describe("Logo Retrieval Tests", () => {
      test("should return presigned URL for workspace with logo", async () => {
        ...
        mockS3Service.generatePresignedDownloadUrl.mockResolvedValue("https://s3.amazonaws.com/download-url");
      ...
      test("should generate 1 hour expiry for download URL", async () => {
        ...
        expect(mockS3Service.generatePresignedDownloadUrl).toHaveBeenCalledWith("workspace-logos/test/123.jpg", 3600);
      });
    });
  });
  ...
  describe("DELETE /api/workspaces/[slug]/settings/image", () => {
    describe("Authentication Tests", () => {
      test("should return 401 for unauthenticated request", async () => {
        const request = createDeleteRequest("http://localhost:3000/api/workspaces/test/settings/image");
