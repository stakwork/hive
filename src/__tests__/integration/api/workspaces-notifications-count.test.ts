...
const TestHelpers = {
  ...
  createAuthenticatedGetRequest: (slug: string, user: { id: string; email?: string | null; name?: string | null }) => {
    const headers = new Headers();
    headers.set("x-middleware-auth-status", "authenticated");
    headers.set("x-middleware-user-id", user.id);
    headers.set("x-middleware-user-email", user.email || "test@example.com");
    headers.set("x-middleware-user-name", user.name || "Test User");
    return new NextRequest(`http://localhost:3000/api/workspaces/${slug}/tasks/notifications-count`, {
      method: "GET",
      headers,
    });
...
describe("GET /api/workspaces/[slug]/tasks/notifications-count - Integration Tests", () => {
  ...
  describe("Authentication", () => {
    ...
    test("should proceed with valid authenticated session", async () => {
      const request = TestHelpers.createAuthenticatedGetRequest(testSlug, testUser);
      const response = await GET(request, { params: Promise.resolve({ slug: testSlug }) });
  ...
  describe("Authorization", () => {
    ...
    test("should allow workspace member to access notification count", async () => {
      ...
      const request = TestHelpers.createAuthenticatedGetRequest(testSlug, memberUser);
      const response = await GET(request, { params: Promise.resolve({ slug: testSlug }) });
    ...
    test("should return 403 for non-member user", async () => {
      ...
      const request = TestHelpers.createAuthenticatedGetRequest(testSlug, outsiderUser);
      const response = await GET(request, { params: Promise.resolve({ slug: testSlug }) });
    ...
    test("should return 404 for non-existent workspace", async () => {
      const request = TestHelpers.createAuthenticatedGetRequest("non-existent-workspace", testUser);
  ...
  describe("Database Integrity", () => {
    ...
    test("should handle concurrent requests correctly", async () => {
      ...
      const request1 = TestHelpers.createAuthenticatedGetRequest(testSlug, testUser);
      const request2 = TestHelpers.createAuthenticatedGetRequest(testSlug, testUser);
