...
describe("useMermaidPaste", () => {
  ...
  it("appends new elements without removing existing ones", async () => {
    const existingElement = {
      id: "user-drawn",
      x: 0,
      y: 0,
    ...
    const elements = updateCall.elements as Array<{ id: string }>;

    // Existing element is preserved
    expect(elements.some((e) => e.id === "user-drawn")).toBe(true);
    // New elements are appended
    expect(elements.length).toBeGreaterThan(1);
  });

  it("two sequential pastes both append: second paste preserves first paste elements", async () => {
    renderMermaidPasteHook();

    // First paste
    await fireAndFlush(makePasteEvent(SIMPLE_GRAPH));
    const firstElements = excalidrawAPI.updateScene.mock.calls[0][0].elements as unknown[];

    // Simulate canvas now containing first paste elements
    excalidrawAPI.getSceneElements.mockReturnValue(firstElements as never);

    // Second paste
    await fireAndFlush(makePasteEvent(SIMPLE_GRAPH));
    const secondElements = excalidrawAPI.updateScene.mock.calls[1][0].elements as unknown[];

    // Second paste should contain all first paste elements plus new ones
    expect(secondElements.length).toBeGreaterThan(firstElements.length);
    for (const el of firstElements) {
      expect(secondElements).toContainEqual(el);
    }
  });
});
