...
vi.mock("@/app/w/[slug]/task/[...taskParams]/components/AgentChatMessage", () => ({
  AgentChatMessage: () => <div>AgentChatMessage</div>,
}));
...
vi.mock("@/components/ThinkingIndicator", () => ({
  ThinkingIndicator: () => <div>ThinkingIndicator</div>,
}));

describe("AgentChatArea - Mobile Header Sticky Behavior", () => {
  const defaultProps = {
    messages: [],
    onSend: vi.fn(),
  };
  ...
  it("should apply fixed top-0 classes to header and pt-16 to messages container when isMobile=true and taskTitle exists", () => {
    ...
    const { container } = render(
      <AgentChatArea {...defaultProps} taskTitle="Test Task Title" workspaceSlug="test-workspace" />
    );

    // Find the header motion.div - it should have fixed top-0 classes
  ...
  it("should NOT render header or apply pt-16 when isMobile=true but taskTitle is null", () => {
    ...
    const { container } = render(<AgentChatArea {...defaultProps} taskTitle={null} />);
  ...
  it("should NOT render header or apply pt-16 when isMobile=true but taskTitle is undefined", () => {
    ...
    const { container } = render(<AgentChatArea {...defaultProps} taskTitle={undefined} />);
