...
describe("resolveSeedUser", () => {
  ...
  describe("fallback strategies", () => {
    ...
    it("should use fallback 1: most recent GitHubAuth entry", async () => {
      ...
      (mockPrisma.gitHubAuth.findFirst as MockedFunction<any>).mockResolvedValueOnce(mockGitHubAuth); // Direct fallback call
    ...
    it("should use fallback 2: GitHub Account lookup when GitHubAuth fallback fails", async () => {
      ...
      (mockPrisma.gitHubAuth.findFirst as MockedFunction<any>).mockResolvedValue(null); // GitHubAuth fallback fails
