...
describe("POST /api/swarm - Unit Tests", () => {
  ...
  describe("Input Validation", () => {
    ...
    test("should accept graph_mindset workspace without repositoryUrl", async () => {
      mockGetServerSession.mockResolvedValue({
        user: { id: "user-123" },
      });
    ...
    test("should pass workspace_type to createSwarm for graph_mindset", async () => {
      mockGetServerSession.mockResolvedValue({
        user: { id: "user-123" },
      });
      mockValidateWorkspaceAccessById.mockResolvedValue({ hasAccess: true, canAdmin: true });
    ...
    test("should skip SourceControlOrg linking for graph_mindset (no repositoryUrl)", async () => {
      mockGetServerSession.mockResolvedValue({
        user: { id: "user-123" },
      });
      mockValidateWorkspaceAccessById.mockResolvedValue({ hasAccess: true, canAdmin: true });
      ...
      // SourceControlOrg lookup and workspace update should NOT be called
