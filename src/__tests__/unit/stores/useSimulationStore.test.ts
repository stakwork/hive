...
import { setupSimulationMocks } from './__fixtures__/simulationMocks'
...
describe('useSimulationStore - addRadialForce', () => {
  ...
  describe('Radial Force Configuration', () => {
    test('should configure radial force with radius 900 and strength 0.1', () => {
    ...
    test('should center radial force at origin (0, 0, 0)', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
      
      store.addRadialForce()
      
      const [radius, x, y, z] = vi.mocked(d3Force.forceRadial).mock.calls[0]
      expect(x).toBe(0)
      expect(y).toBe(0)
      expect(z).toBe(0)
    })
  ...
  describe('Charge Force Configuration', () => {
    test('should configure charge force with scale-based negative strength', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
      ...
      // Test strength calculation for node with scale
      const nodeWithScale = { ref_id: 'test', scale: 2 }
      expect(strengthCallback(nodeWithScale)).toBe(-200) // 2 * -100
      
      // Test strength calculation for node without scale
      const nodeWithoutScale = { ref_id: 'test2' }
      expect(strengthCallback(nodeWithoutScale)).toBe(-100) // 1 * -100
    })
    
    test('should use scale of 1 for nodes with null scale', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
  ...
  describe('Directional Forces Configuration', () => {
    test('should remove y force by setting it to null', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
      
      store.addRadialForce()
      
      // Verify 'y' force is explicitly set to null before other configurations
      const yForceCall = mockSimulation.force.mock.calls.find(
        (call: any) => call[0] === 'y' && call[1] === null
      )
      expect(yForceCall).toBeDefined()
    })
    ...
    test('should configure forceX with strength 0', () => {
      ...
      expect(d3Force.forceX).toHaveBeenCalled()
      expect(mockStrength).toHaveBeenCalledWith(0)
      expect(mockSimulation.force).toHaveBeenCalledWith('x', mockForces.x)
    })
    ...
    test('should configure forceY with strength 0', () => {
      ...
      expect(d3Force.forceY).toHaveBeenCalled()
      expect(mockStrength).toHaveBeenCalledWith(0)
      expect(mockSimulation.force).toHaveBeenCalledWith('y', mockForces.y)
    })
    ...
    test('should configure forceZ with strength 0', () => {
      ...
      expect(d3Force.forceZ).toHaveBeenCalled()
      expect(mockStrength).toHaveBeenCalledWith(0)
      expect(mockSimulation.force).toHaveBeenCalledWith('z', mockForces.z)
    })
  })
  ...
  describe('Node Position Reset', () => {
    ...
    test('should preserve node ref_id and other properties during reset', () => {
      ...
      expect(capturedNodes[0].ref_id).toBe('node1')
      expect(capturedNodes[0].scale).toBe(2)
      expect(capturedNodes[0].node_type).toBe('Function')
    })
  })
  ...
  describe('Collision Detection', () => {
    test('should calculate collide radius based on node scale', () => {
      ...
      // Test node with scale
      const nodeWithScale = { ref_id: 'test', scale: 2 }
      expect(radiusCallback(nodeWithScale)).toBe(160) // 2 * 80
      
      // Test node without scale (defaults to 1)
      const nodeWithoutScale = { ref_id: 'test2' }
      expect(radiusCallback(nodeWithoutScale)).toBe(80) // 1 * 80
    })
  ...
  describe('Force Configuration Order', () => {
    test('should set all forces in correct order', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
      ...
      // First call should be removing 'y' force, then setting other forces
      expect(forceSetterCalls[0]).toBe('y') // Remove y force first
      expect(forceSetterCalls.slice(1)).toEqual(['radial', 'center', 'charge', 'x', 'y', 'z', 'link', 'collide'])
    })
  })
  ...
  describe('Integration with setForces', () => {
    ...
    test('should be called for default graphStyle', () => {
      vi.mocked(useGraphStore.getState).mockReturnValue({
      ...
      // Default case should call addRadialForce
      expect(addRadialForceSpy).toHaveBeenCalled()
    })
  })
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle nodes with missing properties', () => {
      const minimalNodes = [{ ref_id: 'node1' }]
      ...
      // Verify reset properties are added
      expect(capturedNodes[0]).toHaveProperty('fx', null)
      expect(capturedNodes[0]).toHaveProperty('fy', null)
      expect(capturedNodes[0]).toHaveProperty('fz', null)
    })
    
    test('should handle nodes with various scale values', () => {
      const store = useSimulationStore.getState()
      store.simulation = mockSimulation
      
      let chargeCallback: any
      let radiusCallback: any
      ...
      // Test zero scale (0 is falsy, so falls back to 1)
      expect(chargeCallback({ scale: 0 })).toBe(-100)
      expect(radiusCallback({ scale: 0 })).toBe(80)
      
      // Test fractional scale
      expect(chargeCallback({ scale: 0.5 })).toBe(-50)
      expect(radiusCallback({ scale: 0.5 })).toBe(40)
      
      // Test large scale
      expect(chargeCallback({ scale: 10 })).toBe(-1000)
      expect(radiusCallback({ scale: 10 })).toBe(800)
    })
  })
})