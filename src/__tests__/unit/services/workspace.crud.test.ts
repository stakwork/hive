...
describe("Workspace CRUD Operations", () => {
  ...
  describe("createWorkspace", () => {
    ...
    test("should throw error for invalid slug", async () => {
      const invalidData = { ...mockWorkspaceData, slug: "_invalid_slug" };
    ...
    test("should return workspace with owner access", async () => {
      ...
      expect(db.workspace.findFirst).toHaveBeenCalledWith({
        ...
        include: {
          ...
          swarm: { select: { id: true, status: true, ingestRefId: true, poolState: true, podState: true, containerFilesSetUp: true, staktrakInjected: true, swarmUrl: true } },
