...
describe("validateWorkspaceSlug", () => {
  ...
  describe("Invalid format", () => {
    ...
    test("should accept slug with underscores and numbers", () => {
      const result = validateWorkspaceSlug("test_workspace_123");
      expect(result).toEqual({ isValid: true });
    });

    test("should accept slug with mixed hyphens and underscores", () => {
      const result = validateWorkspaceSlug("test-workspace_123");
      expect(result).toEqual({ isValid: true });
    });
    ...
    test("should accept slug with consecutive underscores", () => {
      const result = validateWorkspaceSlug("work__space");
      expect(result).toEqual({ isValid: true });
    });
