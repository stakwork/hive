...
import { resetMockProposals, mockConceptDocs } from "@/app/api/mock/stakgraph/gitree/proposals/fixtures";
...
describe("POST /api/mock/stakgraph/gitree/proposals/[id]/accept", () => {
  ...
  it("returns 409 { code: 'stale_base' } when baseDocs doesn't match current docs", async () => {
    ...
    // No `error` field — matches swarm contract
  ...
  it("verifies stale fixture baseDocs actually differs from current mockConceptDocs", () => {
    ...
    const stalePropBaseDocs =
      "Handles JWT flows for user authentication. (outdated snapshot)";
    const currentDocs = mockConceptDocs["stakwork/hive/auth"];
    expect(stalePropBaseDocs).not.toBe(currentDocs);
  });

  // ── already decided ────────────────────────────────────────────────────────

  it("returns 409 { status } when proposal is already accepted (no error field)", async () => {
  ...
  it("returns 409 { status } when proposal is already rejected (no error field)", async () => {
...
describe("POST /api/mock/stakgraph/gitree/proposals/[id]/reject", () => {
  ...
  it("returns 409 { status } when already accepted (no error field)", async () => {
    // Accept first
    await acceptPost(
      makeActionReq("proposal-merge-1", "accept", { decidedBy: "u" }),
      { params: makeParams("proposal-merge-1") },
  ...
  it("returns 409 { status } when already rejected", async () => {
    // Reject once
    await rejectPost(
      makeActionReq("proposal-update-1", "reject", { decidedBy: "u" }),
      { params: makeParams("proposal-update-1") },
