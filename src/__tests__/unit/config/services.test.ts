...
describe("getServiceConfig", () => {
  describe("Known services", () => {
    it("should return correct configuration for stakwork service", () => {
      ...
      expect(config.baseURL).toBe(process.env.STAKWORK_BASE_URL || "https://jobs.stakwork.com/api/v1");
    ...
    it("should return correct configuration for poolManager service", () => {
      ...
      expect(config.baseURL).toBe(process.env.POOL_MANAGER_BASE_URL || "https://workspaces.sphinx.chat/api");
      expect(config.headers).toHaveProperty("Content-Type", "application/json");
    });
  ...
  describe("Configuration structure validation", () => {
    ...
    it.each(knownServices)("should return ServiceConfig interface-compliant object for %s service", (serviceName) => {
  ...
  describe("Unknown services", () => {
    ...
    it("should throw error with correct message format for unknown service", () => {
      ...
      expect(() => getServiceConfig(unknownService)).toThrow("Unknown service: invalidService");
    });
    ...
    it.each(["invalid", "notFound", "random", "", "null", "undefined"])(
      "should throw error for invalid service name: %s",
      (invalidService) => {
        expect(() => getServiceConfig(invalidService as any)).toThrow(`Unknown service: ${invalidService}`);
      },
    );
  });
